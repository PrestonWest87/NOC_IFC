...
def fetch_live_crimes():
    """Fetches live 48-hour crime data via SpotCrime RSS for the regional NOC footprint."""
    ...
    # Coordinates for Little Rock / Central AR footprint
    ...
    for entry in feed.entries:
        try:
            ...
            if any(k in title for k in ["THEFT", "BURGLARY", "ROBBERY"]):
                category = "Theft / Possible Copper or Asset Loss"
                severity = "High"
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ✅ CRIME WORKER: {len(crimes)} incidents logged in last 48 hours.")
