...
def job_daily_email_unified_brief():
    ...
    try:
        ...
        #  FIX: Keep everything inside a SINGLE session block
        with SessionLocal() as session:
            latest_internal = session.query(InternalRiskSnapshot).order_by(InternalRiskSnapshot.timestamp.desc()).first()
            active_nws = session.query(RegionalHazard).count()
            ...
            if brief_text and "AI is currently disabled" not in brief_text:
                ...
                #  FIX: Only bail out of the EMAIL sequence if recipients are missing (After the brief is safely saved)
                if not recipients:
                    log("[WARN] No SMTP recipient configured. Unified brief saved, but skipping email dispatch.", "SYSTEM")
                    return
...
def job_tiered_alert_escalation():
    ...
    with SessionLocal() as db:
        ...
        def is_node_on_cooldown(node_name, cooldown_hours):
            """Checks if a specific node has been dispatched recently to prevent flap spam."""
        ...
        def get_tier(alert):
            """Extracts and normalizes the alert tier."""
        ...
        for site, data in incidents.items():
            ...
            # Sort by time to establish Patient Zero
            ...
            if duration_active >= timedelta(minutes=wait_minutes):
                ...
                if target_tier == "p2-high" and not alert_is_day: base_body += "Requirement: Positive Handoff (No ONPAGE)\n"
                ...
                # -> B & C. AFTER-HOURS ONLY (NOC NOTIFICATIONS AND ONPAGES)
                if not alert_is_day:
