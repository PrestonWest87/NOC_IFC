...
* **Operational Impact:**
    ...
    * `PRAGMA cache_size=-16000`: Allocates 16MB of RAM to the database cache, reducing disk I/O for frequent queries while optimizing memory footprint.
    * `PRAGMA temp_store=MEMORY` & `PRAGMA mmap_size=268435456`: Forces temporary tables and memory-mapped files into RAM (capped at 256MB), executing complex `GROUP BY` and geospatial queries instantly.
...
* **Purpose:** Stores chronological incident logs and running notes isolated by operational role for shift handoffs.
* **Key Fields:** * `author_role`: Enforces role-based isolation of logs (e.g., TOC vs. NOC).
...
* **Purpose:** Tracks historical daily snapshots of operational risk. Used by the Executive Dashboard to generate 14-Day Threat Deviation Trend charts comparing current `cyber_points` and `physical_points` against baselines.
...
* **Key Fields:** * `lat`, `lon` (for Haversine radius calculations), and `priority` (an integer denoting criticality).
    * `under_maintenance`, `maintenance_etr`, `maintenance_reason`: Suppresses alert correlation and informs dispatch workflows if a site is undergoing planned work.
...
* **Purpose:** High-speed cache for raw mapping geometry feeds (SPC Convective Outlooks, NWS Warnings), accelerating rendering in the PyDeck geospatial grid.
...
* **Operational Flow:**
    ...
    3.  **Silent Migrations:** Executes a series of non-blocking `ALTER TABLE` statements wrapped in `try...except` blocks to natively append new columns (like `under_maintenance` or `author_role`) to legacy databases without wiping data.
    4.  **RBAC Auto-Healing:** Hardcodes the `all_pages` and `all_actions` arrays utilizing the newest descriptive nomenclature. It checks for the existence of the `admin` and `analyst` roles, forcefully appending new operational capabilities like `"Action: Dispatch RCA Tickets"`, `"Action: Manage Site Maintenance"`, and `"Tab: Shift Log -> Active Shift"`.
    5.  **Admin Seeding:** If the `User` table is entirely empty, it generates a default `admin` user with a securely hashed bcrypt password.
    6.  **Transactional Integrity:** Operates within a strict `try...except...finally` block, executing `session.rollback()` on failure and `session.close()` to ensure the connection pool is not depleted during bootstrap.
