...
export function DashboardPage() {
  ...
  const handleDispatchReport = async () => {
    ...
    try {
      await api.post("/admin/config", {
        ...
        email: dispatchEmail,
        unified_risk: executiveIntel.unified_risk,
        cyber_score: executiveIntel.cyber_score,
        physical_score: executiveIntel.physical_score,
        report: scoringOverview,
      });
  ...
  const handleBroadcastBrief = async () => {
    ...
    try {
      await api.post("/admin/config", {
        action: "broadcast_unified_brief", email: ubEmail,
      });
      alert("Brief transmitted to " + ubEmail);
    } catch { alert("Failed to transmit brief."); }
  };
